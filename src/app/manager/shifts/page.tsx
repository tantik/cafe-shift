...
function createInitialAssignments() {
  return days.reduce<Record<string, Assignment[]>>((result, day) => {
    result[day.key] = employees.slice(0, 6).map((employee, index) => {
  ...
  return (
    <AppShell variant="wide">
      <div className="mx-auto max-w-4xl space-y-4 pb-8">
        ...
        <section className="rounded-2xl border border-amber-100 bg-amber-50 p-4 shadow-sm">
          <h2 className="font-semibold text-slate-900">スタッフ希望メモ</h2>
          <div className="mt-2 space-y-1 text-sm text-slate-700">
            <p>山田: 1シフト希望</p>
            <p>佐藤: 休み希望</p>
          </div>
          <p className="mt-2 text-sm text-slate-500">最終シフトは管理者が調整します。</p>
        </section>
