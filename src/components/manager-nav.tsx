...
const managerNavItems = [
  { href: "/manager", labelKey: "manager.root" },
...
export default function ManagerNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 border-t border-slate-200 bg-white/95 px-3 py-3 shadow-[0_-8px_24px_rgba(15,23,42,0.08)] backdrop-blur-sm">
      <div className="mx-auto grid max-w-[520px] grid-cols-4 gap-1.5 sm:gap-2">
        {managerNavItems.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`rounded-lg border px-2 py-2 text-center text-xs font-bold shadow-sm transition sm:text-sm ${
                isActive
